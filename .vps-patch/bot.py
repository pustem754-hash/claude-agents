...
# Абсолютные unix-пути до .pdf. Не захватываем пробелы и типовые ограничители.
PDF_PATH_RE = re.compile(r'(/[^\s`()\[\]<>"\\\']+\.pdf)', re.IGNORECASE)
    ...
    try:
        while True:
            try:
                ...
                # Сеть может моргнуть — не роняем keepalive, просто подождём следующий тик.
...
async def send_answer(msg: Message, answer: str) -> None:
    ...
    # Пока качаем/отправляем PDF, держим индикатор "uploading document".
